...
def showType(type):
    ...
    try:
        ...
        items = session.query(Item).filter_by(type_id=type_id).all()
        if status == 'loggedOut':
            # return list of items of one type with
            # no 'create new item' option
        ...
        return render_template('typeError.html', type=type, types=types,
                               status=status)
...
def addNewItem():
    if 'username' not in login_session:
        ...
        if (not request.form['name'] or not request.form['description'] or
        ...
        if file and allowed_file(file.filename):
            ...
            if item:
                return render_template('preFilledForm.html',
                                       ...
                                       description=request.form['description'],
                                       types=types,
                                       error='This image name already exists',
                                       status='loggedIn')
...
def itemToEdit(item_id):
    try:
        ...
        if editedItem.user_id == login_session['user_id']:
            types = session.query(Type).all()
            return render_template('editedItemForm.html', types=types,
                                   item=editedItem, status='loggedIn')
        else:
        ...
        if editedItem.user_id == login_session['user_id']:
            if (not request.form['name'] or
                not request.form['description'] or
                    not request.form['type']):
                return render_template('preFilledFormNoImage.html',
                                       name=request.form['name'],
                                       description=request.form['description'],
                                       error='Please fill out all fields',
                                       types=types, status='loggedIn')
